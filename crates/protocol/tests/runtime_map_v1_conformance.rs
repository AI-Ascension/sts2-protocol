...
#[test]
fn runtime_map_v1_text_controls_fail_schema_and_typed_validation() {
    ...
    for field in ["game_build", "mod_version", "reason"] {
        for control in ['\0', '\n', '\u{1f}', '\u{7f}', '\u{85}', '\u{9f}'] {
            for text in [format!("a{control}b"), format!("a{control}")] {
                let mut value: Value = serde_json::from_str(SNAPSHOT).expect("snapshot JSON");
                if field == "reason" {
                    value["completeness"] = json!("incomplete");
                }
                value[field] = json!(text);
                assert!(
                    !validator.is_valid(&value),
                    "schema accepted {field}: {text:?}"
                );
                assert!(decode_map_snapshot(&serde_json::to_vec(&value).expect("JSON")).is_err());
            }
        }
    }
}
...
#[test]
fn runtime_map_v1_utf8_byte_bounds_supplement_schema_character_bounds() {
    let schema: Value = serde_json::from_str(SOURCE_SCHEMA).expect("schema JSON");
    let validator = jsonschema::draft202012::options()
        .build(&schema)
        .expect("schema");
    for (field, maximum) in [("game_build", 128), ("mod_version", 128), ("reason", 256)] {
        for character in ["a", "é", "界", "😀"] {
            let text = character.repeat(maximum / character.len())
                + &"a".repeat(maximum % character.len());
            ...
            value[field] = json!(text);
            assert!(validator.is_valid(&value));
            assert!(decode_map_snapshot(&serde_json::to_vec(&value).expect("JSON")).is_ok());
            value[field] = json!(text + "a");
            assert!(decode_map_snapshot(&serde_json::to_vec(&value).expect("JSON")).is_err());
            // Multibyte overflow can remain structurally valid; the byte check is mandatory.
            assert_eq!(validator.is_valid(&value), character.len() > 1);
        }
    }
}
