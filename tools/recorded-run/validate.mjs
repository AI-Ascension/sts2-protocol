...
export const schemaBytes = readFileSync(new URL("../../artifacts/recorded-run-bundle-v1-candidate3/schema.json", import.meta.url));
...
function countRecords(data, budget) {
  if (!data.length) return;
  insist(data.at(-1)===10, "partial_final_record");
  let start=0;
  for (let i=0;i<data.length;i++) if (data[i]===10) {
    insist(i>start && i-start<=LIMITS.record, "record_limit");
    insist(budget.remaining>0, "record_count");
    budget.remaining--;start=i+1;
  }
}
function ndjson(data, budget) {
  if (!data.length) return [];
  insist(data.at(-1)===10, "partial_final_record");
...
export function validateBundle(input) {
  ...
  const eventBytes=entries.get("records/events.ndjson");
  const accountingBytes=entries.get("records/accounting.ndjson")??new Uint8Array(0);
  // Reject the combined count/line limits before materializing any record.
  const preflight={remaining:LIMITS.records};
  countRecords(eventBytes,preflight);countRecords(accountingBytes,preflight);
  const parsing={remaining:LIMITS.records};
  const events=ndjson(eventBytes,parsing),accounting=ndjson(accountingBytes,parsing);
