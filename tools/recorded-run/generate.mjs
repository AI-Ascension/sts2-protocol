...
const root=fileURLToPath(new URL("../../artifacts/recorded-run-bundle-v1-candidate3/",import.meta.url));
...
const caseDocument={profile:"recorded-run-bundle-v1",version:"1.0.0-candidate.3",schema_sha256:schemaDigest,
...
const manifest={artifact:"recorded-run-bundle-v1",status:"candidate",version:"1.0.0-candidate.3",
...
const toolingPaths=readdirSync(join(repository,"tools/recorded-run")).filter(p=>p.endsWith(".mjs"))
  .map(p=>"tools/recorded-run/"+p).concat(["conformance/cases/recorded-run-bundle-v1-candidate3.json",
    "crates/protocol/tests/recorded_run_conformance.rs","docs/decisions/0035-recorded-run-candidate3-review.md"]).sort();
