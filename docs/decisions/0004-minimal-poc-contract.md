...
bundle, with manifest, copied schema, two golden messages, one invalid typed-action fixture, and
SHA-256 inventory. Downstream targets consume copied artifact bytes and implement local mappings;
none imports `sts2-protocol` Rust modules or a generic common crate.
...
Target-local conformance validates source/artifact JSON equivalence, exact golden bytes, metadata,
error identity, and the invalid fixture. Consumer PRs must validate their copied manifest and digest.
Any live host, network, provider, MCP session, gateway process, or publication remains unverified.
